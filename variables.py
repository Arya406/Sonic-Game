...
states_duck = [
    pygame.image.load("images/duck1.png").convert_alpha(),
    pygame.image.load("images/duck2.png").convert_alpha()
]
...
grass_3_rect = Environment(grass_surface.get_rect(topleft=(width // 2, height)), grass_surface, "grass")

cloud_surface = pygame.image.load("images/cloud.png").convert_alpha()
cloud_rect = Environment(
    cloud_surface.get_rect(
        topleft=(
            width + randint(0, 500),
...
sonic_jump_rect = Sonic(
    sonic_jump_surface.get_rect(
        topleft=(100, height - 200 - 144 * 4)
    )
)

sonic_1_rect = Sonic(
    states_sonic[0][0].get_rect(
...
last_score_surface = score_font.render("Last score : 0", True, (0, 0, 0))
...
pseudo_font = pygame.font.Font(FONTPATH, int(width // 3 // max(len(NAMEID), 1)))
pseudo_surface = pseudo_font.render(NAMEID, True, (0, 0, 0))
